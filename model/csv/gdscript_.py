...
g_UUID = 0
g_makeDeep = 0
g_exportTypeString = ["", "EXPORT_BOOL", "EXPORT_NUM", "EXPORT_STRING", "EXPORT_ARRAY", "EXPORT_MAP", "EXPORT_CSV",
					  "EXPORT_NIL"]
g_space = 0
g_spaceIndent = 4
...
if __name__ == '__main__':
	...
	for file in getFiles(input_path, '.csv'):
		with open(file["path"], 'r', encoding='utf-8') as f:
			...
			write_line_str(file_export, "class_name Csv%s" % (export_table.capitalize()))
			write_table_str(file_export, "const var _data = {")
			keys = ""
	...
	write_line_str(file_export, "class_name CsvHelp\n")
	for v in csv_list:
		write_line_str(file_export, 'const var %s = preload("res:\\\\scripts/config/%s").new()' % (v, v))
	file_export.close()
